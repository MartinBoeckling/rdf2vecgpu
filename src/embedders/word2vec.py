...
class OrderAwareSkipgram(L.LightningModule):
    ...
    def forward(self, center, context, distances, negative, neg_distances=None):
        ...
        for i in range(batch_size):
            ...
            context_embed = self.output_embeddings[pos_idx](context[i:i+1])  # (1, embed_dim)
            pos_embed = self.position_embeddings(pos_indices[i:i+1])  # (1, embed_dim)
        ...
        if neg_distances is None:
            ...
            neg_distances = torch.randint(-self.max_distance, self.max_distance + 1, 
                                        (batch_size, self.neg_samples), device=center.device)
        
        neg_pos_indices = self._get_position_index(neg_distances)  # (batch_size, neg_samples)
        ...
        for i in range(batch_size):
            ...
            for j in range(self.neg_samples):
                ...
                neg_embed = self.output_embeddings[pos_idx](negative[i:i+1, j:j+1])  # (1, 1, embed_dim)
                neg_pos_embed = self.position_embeddings(neg_pos_indices[i:i+1, j:j+1])  # (1, 1, embed_dim)
                
                # Combine negative context and position embeddings
                combined_neg_embed = neg_embed.squeeze(1) + neg_pos_embed.squeeze(1)  # (1, embed_dim)
    ...
    def training_step(self, batch, batch_idx):
        ...
        negative = torch.randint(0, self.vocab_size, (batch_size, self.neg_samples), device=self.device)
        
        # Generate negative distances if not provided
        if neg_distances is None:
            neg_distances = torch.randint(-self.max_distance, self.max_distance + 1, 
                                        (batch_size, self.neg_samples), device=self.device)
...
class OrderAwareCBOW(L.LightningModule):
    ...
    def forward(self, context_words, context_distances, center, negative, neg_distances=None):
        """
        Args:
        ...
        for i in range(batch_size):
            ...
            for j in range(context_size):
                ...
                ctx_embed = self.in_embeddings[pos_idx](context_words[i:i+1, j:j+1])  # (1, 1, embed_dim)
                pos_embed = self.position_embeddings(self._get_position_index(distance).unsqueeze(0))  # (1, embed_dim)
            ...
            avg_context = torch.mean(torch.cat(batch_context_embeds, dim=0), dim=0, keepdim=True)  # (1, embed_dim)
        ...
        # Compute negative scores
        negative_embed = self.output_embeddings(negative)  # (batch_size, neg_samples, embed_dim)
        
        # Expand context embeddings to match negative samples shape
        context_embeds_expanded = context_embeds.unsqueeze(1)  # (batch_size, 1, embed_dim)
        
        # Compute negative logits
        neg_logits = (context_embeds_expanded * negative_embed).sum(dim=2)  # (batch_size, neg_samples)
        neg_targets = torch.zeros_like(neg_logits)
        neg_loss = self.loss_fn(neg_logits, neg_targets).sum(dim=1)
    ...
    def training_step(self, batch, batch_idx):
        if len(batch) == 3:
            ...
            context_words, context_distances, center, _ = batch  # Ignore neg_distances if provided
        ...
        with torch.no_grad():
            ...
            for i in range(batch_size):
                ...
                for j in range(context_size):
                    ...
                    ctx_embed = self.in_embeddings[pos_idx](context_words[i:i+1, j:j+1])
                    pos_embed = self.position_embeddings(self._get_position_index(distance).unsqueeze(0))
                    
                    combined_embed = ctx_embed.squeeze(1) + pos_embed
                    batch_context_embeds.append(combined_embed)
                
                avg_context = torch.mean(torch.cat(batch_context_embeds, dim=0), dim=0, keepdim=True)
                context_embeds.append(avg_context)
            ...
            scores = torch.mm(context_embeds, all_center_embeds.t())  # (batch_size, vocab_size)
